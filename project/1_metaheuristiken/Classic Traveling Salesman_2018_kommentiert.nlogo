...
to setup
 ...
 create-turtles population-size [
   ...
   ;;>>>> das muss nicht zwangsläufig hier geschehen, sondern wäre (!!je nach Implementierung der Prozedur "calculate-distance"!!) ein paar Zeilen weiter unten besser aufgehoben <<<<<< Changed by Tim
  ...
  ask turtles [
   ...
   calculate-winner-looser-fitness
...
to calculate-fitness
    ...
    ;;set winner max-one-of turtles [1 / fitness] ;;in this case the winner is the turtle traveling the least distance
    ;;set looser min-one-of turtles [1 / fitness] ;;  hier wird auch das schlechteste Individuum gespeichert (im weiteren Verlauf bisher aber nicht wirklich verwendet)
end

to calculate-winner-looser-fitness
  set winner max-one-of turtles [1 / fitness] ;;in this case the winner is the turtle traveling the least distance
  set looser min-one-of turtles [1 / fitness] ;;  hier wird auch das schlechteste Individuum gespeichert (im weiteren Verlauf bisher aber nicht wirklich verwendet)
end
;;>>>>>>>>
...
to create-new-generation
 ...
 ;;(und später wieder hinzugefügt)
  ...
  ;;>>>>>> sollte die Fitnessbewertung angepasst worden sein, ist hier zu prüfen, ob statt "min-one-of" besser "max-one-of" zu wählen ist <<<<<< Changed by Tim
  ;;>>>>>> wenn die Selektionsmethode geändert wird, wäre statt der obigen Zeilen eine Anpassung bzw. Neuimplementierung erforderlich <<<<<<
  ;; Elitismus zur Auswahl der beiden Elternpaare:
  let parent1-p 0
  let parent2-p 0
  ;ifelse elitism? ;;TODO: Wie genau werden die Eltern bei Elitismus gebildet? Immer von den besten Beiden oder nur einer der beste oder oder oder...
   ;   [
    ;    let best min-n-of 2 old-generation [fitness]
     ;   set parent1-p max-one-of best [fitness]
      ;  set parent2-p min-one-of best [fitness]
      ;]
      ;[
...
ask turtles [
    ...
    ifelse self = winner and elitism?
    []
    [
