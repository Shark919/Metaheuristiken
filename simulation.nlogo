...
;; decide, influenced by prospect theory
;; reference: last known price
;; fear of having to pay more than that, even if probability is low
;; gain for paying a little less is lower, than the fear of loss
;; w(probGain)*v(gain) + w(probLoss)*v(loss)
;; with probGain = .40, probLoss = .40, probNeutral = .20
;; gain/loss = 10cent *100l = 10€
;; prospectTheory: fear of loss
;; DECISON: I chose gasstation based on distance
;; when I am near to destination, Descion: refuel with gain/loss or go for a risky move
